...
| 구분 | 주소 | 비고 |
|---|---|---|
| **운영 게이트웨이** | `https://api.sj-lab.co.kr` | 클러스터 NodePort 30089 인그레스 경유 |
| **연결 확인 테스트 URL** | `https://api.sj-lab.co.kr/map/check` | 게이트웨이 및 백엔드 헬스체크 정상 메시지 반환 (`HTTP 200`) |
| **로컬 개발 게이트웨이** | `http://localhost:8100` | 로컬 스택 단일 진입점 |
| **로컬 테스트 URL** | `http://localhost:8100/map/check` | 로컬 라우팅 확인용 |

> ⚠️ **게이트웨이 루트(`/`) 접속 시 주의**:
> 게이트웨이 기본 루트(`https://api.sj-lab.co.kr/`)는 라우팅 룰이 정의되어 있지 않아 404가 발생합니다. 게이트웨이의 정상 작동 및 백엔드 라우팅 연결 여부는 **[https://api.sj-lab.co.kr/map/check](https://api.sj-lab.co.kr/map/check)** 로 접속하여 확인하십시오.
